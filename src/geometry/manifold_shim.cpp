...
// `ring_lengths`. A counter-clockwise ring bounds material and a clockwise one
// inside it is a hole, which the triangulation leaves empty rather than
// covering. `epsilon` is passed to Manifold unchanged, where a negative value
// selects its own tolerance. At most `capacity` triangles are written to
// `triangles` as vertex indices into the concatenated point array, and the
// number written is stored in `written`.
//
// Fewer than three points in total is not an error and writes nothing.
int vertexTriangulatePolygons(const double *xy, const uint32_t *ring_lengths,
                              size_t ring_count, double epsilon,
                              uint32_t *triangles, size_t capacity,
                              size_t *written) {
  *written = 0;
  if (ring_count == 0) return VERTEX_TRIANGULATE_OK;
  size_t total = 0;
  for (size_t i = 0; i < ring_count; ++i) total += ring_lengths[i];
  if (total < 3) return VERTEX_TRIANGULATE_OK;

  const size_t ring_size = manifold_simple_polygon_size();
  void *rings_mem = std::malloc(ring_count * ring_size);
  ManifoldSimplePolygon **rings = static_cast<ManifoldSimplePolygon **>(
      std::malloc(ring_count * sizeof(ManifoldSimplePolygon *)));
  void *polygons_mem = std::malloc(manifold_polygons_size());
  void *triangulation_mem = std::malloc(manifold_triangulation_size());
  if (rings_mem == nullptr || rings == nullptr || polygons_mem == nullptr ||
  ...
  try {
    ...
    for (size_t i = 0; i < ring_count; ++i) {
      ...
      rings[i] = manifold_simple_polygon(
          ...
          ring_lengths[i]);
      cursor += 2 * ring_lengths[i];
      ++built;
    }
    polygons = manifold_polygons(polygons_mem, rings, ring_count);
